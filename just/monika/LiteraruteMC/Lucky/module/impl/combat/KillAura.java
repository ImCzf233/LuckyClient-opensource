...
public final class KillAura extends Module {
    ...
    private final EventListener<MotionEvent> onMotion = e -> {
        ...
        if (LuckyClient.INSTANCE.isToggled(Scaffold.class) || mc.thePlayer.isDead || mc.thePlayer.isSpectator())
            return;
        if (!targets.isEmpty()) {
            ...
            if (e.isPre()) {
                ...
                if (rotationsSetting.getSetting("Resolver").isEnabled()) {
                    ...
                    if (Math.abs(target.posX - target.lastTickPosX) > 0.50 || Math.abs(target.posZ - target.lastTickPosZ) > 0.50) {
                        ...
                        if (debug.isEnabled()) {
                            mc.thePlayer.addChatComponentMessage(new ChatComponentText("LuckyClient: resloved target hitbox at " + target.posX + "," + target.posY + "," + target.posZ));
                        }
                    }
                }
