package just.monika.LiteraruteMC.Lucky.utils.spotify;
...
public class SpotifyAPI implements Utils {
    ...
    //.setLenient()
    public final static File CLIENT_ID_DIR = new File(LuckyClient.DIRECTORY, "SpotifyID.json");
    ...
    private final SpotifyCallBack callback = code -> {
        ChatUtils.print("Spotify", EnumChatFormatting.GREEN, "Connecting to Spotify...");
    ...
    public void startConnection() {
        if (!authenticated) {
            try {
                ...
                LuckyClient.INSTANCE.getExecutorService().submit(() -> {
                    try {
                        if (callbackServer != null) {
                            // Close the server if the module was disabled and re-enabled, to prevent already bound exception.
                            callbackServer.stop(0);
                        }
                        ChatUtils.print("Spotify", EnumChatFormatting.GREEN, "Please allow access to the application.");
                        ...
                        callbackServer.createContext("/", context -> {
                            ...
                            final String messageSuccess = context.getRequestURI().getQuery().contains("code")
                                    ? "Successfully authorized.\nYou can now close this window, have fun on LuckyClient!"
    ...
    public void skipToPreviousTrack() {
        try {
            ...
            ChatUtils.print("Spotify", EnumChatFormatting.RED, e.getMessage());
        }
    }
