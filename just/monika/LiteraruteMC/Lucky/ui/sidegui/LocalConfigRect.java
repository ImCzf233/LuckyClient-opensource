...
import just.monika.LiteraruteMC.Lucky.config.LocalConfig;
...
public class LocalConfigRect extends GuiPanel {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks, int alpha) {
        ...
        FontUtil.iconFont26.drawString(FontUtil.SAVE, updateX, updateY,
                ColorUtil.interpolateColor(textColor, LuckyClient.INSTANCE.getClientColor(), (float) hoverUpdateAnimation.getOutput()));

        if (hoveringUpdate || !hoverUpdateAnimation.isDone()) {
            int updateColor = ColorUtil.interpolateColor(ColorUtil.applyOpacity(LuckyClient.INSTANCE.getClientColor(), 0),
                    LuckyClient.INSTANCE.getClientColor(), (float) hoverUpdateAnimation.getOutput());
        ...
        FontUtil.iconFont26.drawString(FontUtil.LOAD, loadX, loadY,
                ColorUtil.interpolateColor(textColor, LuckyClient.INSTANCE.getAlternateClientColor(), (float) hoverLoadAnimation.getOutput()));

        if (hoveringLoad || !hoverLoadAnimation.isDone()) {
            int loadColor = ColorUtil.interpolateColor(ColorUtil.applyOpacity(LuckyClient.INSTANCE.getAlternateClientColor(), 0),
                    LuckyClient.INSTANCE.getAlternateClientColor(), (float) hoverLoadAnimation.getOutput());
    ...
    public void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (button == 0) {
            if (hoveringDelete) {
                if (LuckyClient.INSTANCE.getConfigManager().delete(config.getName())) {
            ...
            if (hoveringUpdate) {
                if (LuckyClient.INSTANCE.getConfigManager().saveConfig(config.getName())) {
            ...
            if (hoveringLoad) {
                if (LuckyClient.INSTANCE.getConfigManager().loadConfig(LuckyClient.INSTANCE.getConfigManager().readConfigData(config.getFile().toPath()))) {
