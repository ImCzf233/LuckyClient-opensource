...
public class KingGenApi {
    ...
    //.setLenient()
    private final File kingAltData = new File(LuckyClient.DIRECTORY, "KingGen.json");
    ...
    public void refreshKey() {
        ...
        try {
            fileContent = jsonParser.parse(new FileReader(kingAltData)).getAsJsonObject();
        ...
        JsonObject responseObject = jsonParser.parse(request.content()).getAsJsonObject();
    ...
    public final String[] genAlt() {
        ...
        if (key.isEmpty()) {
            try {
                ...
                JsonObject fileContent = jsonParser.parse(reader).getAsJsonObject();
