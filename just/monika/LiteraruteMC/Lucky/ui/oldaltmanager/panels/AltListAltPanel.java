...
import just.monika.LiteraruteMC.Lucky.ui.oldaltmanager.panels.components.impl.AltRect;
...
public class AltListAltPanel implements AltPanel {
    ...
    ArrayList<just.monika.LiteraruteMC.Lucky.ui.oldaltmanager.panels.components.Component> altObjects = new ArrayList<>();
    ArrayList<just.monika.LiteraruteMC.Lucky.ui.oldaltmanager.panels.components.Component> buttons = new ArrayList() {{
        addAll(Arrays.asList(
                new just.monika.LiteraruteMC.Lucky.ui.oldaltmanager.panels.components.impl.Button("Direct Login", 100, 50),
                new just.monika.LiteraruteMC.Lucky.ui.oldaltmanager.panels.components.impl.Button("Delete", 100, 50)
        ));
    }};
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks, Animation initAnimation) {
        ...
        buttons.forEach(button -> {
            ...
            ((just.monika.LiteraruteMC.Lucky.ui.oldaltmanager.panels.components.impl.Button) button).width = width / 2f - 10;
        });

        buttons.get(0).y = (40 + height) - (5 + ((just.monika.LiteraruteMC.Lucky.ui.oldaltmanager.panels.components.impl.Button) buttons.get(0)).height);
...
//        Draw.drawRoundedRect(xAnimation + width - 5, -scroll.getScroll() / scroll.getMaxScroll() * (height - scrollBarHeight) + 40, 3, scrollBarHeight, 1.5f, rectColorInt);//new Color(255, 255, 255, 120).getRGB()
    ...
    public void drawAltHead(Alt alt, float x, float y, float size) {
        ...
        mc.getTextureManager().bindTexture(alt.username == null ? new ResourceLocation("LuckyClient/steve.png") : AbstractClientPlayer.getLocationSkin(alt.username));
        if (alt.email == null) {
